<template>
  <l-map ref='map'
    ...
    <l-geo-json v-if='mapColoringReady'
                ref='mapColoring'
                :geojson='geoJson.japanWithSubAreas'
                :options='{
...
import { IAreaColor, IGeoJsonData, IGeoJsonFeature } from '~/assets/interfaces/parsing/GeoJson'
...
export default class QuakeMap extends Vue {
  ...
  protected mapOptions = hnapi.mapConfig.mapOptions
  protected countriesBgStyle = hnapi.mapConfig.countriesBgStyle
  protected japanBgStyle = hnapi.mapConfig.japanBgStyle
  protected japanBorderStyle = hnapi.mapConfig.japanBorderStyle
  protected japanAreaBorderStyle = hnapi.mapConfig.japanAreaBorderStyle
  protected sWaveCircleStyle = hnapi.mapConfig.sWaveCircleStyle
  protected pWaveCircleStyle = hnapi.mapConfig.pWaveCircleStyle
  ...
  public addMapIntensities(intensity:
    ...
    for (const i in intensity) {
      ...
      const intensityConverted = hnapi.intensityConfig
        .intensityConversion[content.intensity]
      if (content.is_area) {
  ...
  private areaIntensityToColor(
    ...
    for (const i in intensity) {
      areaColoring.colors[i] =
        hnapi.intensityConfig.intensityColors[intensity[i].intensity]
